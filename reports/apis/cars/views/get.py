...
def get_car_report(request, car):
    try:
        ...
        data = {
            ...
            'final_profits': round(final_profits),
...
@api_view(['GET'])
@permission_classes
def car_suggestion(request, car):
    cars_sugg= Car.objects.filter(car_name__icontains= car)
    serializer= CarSerializer(cars_sugg, many= True)
